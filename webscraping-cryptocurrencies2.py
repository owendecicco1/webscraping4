...
for x in range(1,6):
    ...
    corresponding_price_based_on_change =float((current_price * (1-(percent_change_in_last_24_hours/100))))

    print(f"Coin: {coin_name}")
    print(f"Price: ${current_price:,.2f}")
    print(f"Percent change in last 24 hours: {percent_change_in_last_24_hours:,.2f}%")
    print(f"Price based on percent change: ${corresponding_price_based_on_change:,.2f}")
    print()
    print()
...
if coin_name == "Ethereum (ETH)" and current_price > 3000:
    print(textmessage1.status)
if coin_name == "Bitcoin (BTC)" and current_price > 40000:
